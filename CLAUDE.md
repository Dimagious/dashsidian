...
и `countdown` описаны в ТЗ и не написаны. Код, тесты и всё, что видит пользователь,
на английском: проект целится в международный стор. Русский — через i18n, наравне
с любым следующим языком. Ядро под `progress` (`core/aggregate.ts`)
...
9. **Ни одной строки для пользователя мимо `t()`.** Новый текст — сперва ключ
   в `src/i18n/en.ts` (там источник правды), потом перевод в `ru.ts`. Каталог
   языка частичный: недостающий ключ падает в английский, а не в сам ключ.
   Тест проверяет, что наборы ключей и плейсхолдеров совпадают.
