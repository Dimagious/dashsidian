...
function findZipCandidate() {
  ...
  const entries = fs
    ...
    .filter((file) => file.startsWith(`${PLUGIN_ID}-`) && file.endsWith('.zip'));
  ...
  const sorted = entries.sort((a, b) => {
    const versionA = a.match(new RegExp(`${PLUGIN_ID}-([^.]+\\..+?)\\.zip`));
    const versionB = b.match(new RegExp(`${PLUGIN_ID}-([^.]+\\..+?)\\.zip`));
...
function extractTagFromFilename(zipPath) {
  ...
  const match = base.match(new RegExp(`${PLUGIN_ID}-((?:v)?\\d+\\.\\d+\\.\\d+)\\.zip`));
...
function main() {
  ...
  if (!zipPath) {
    problems.push(`No ${PLUGIN_ID}-*.zip found in repository root. Run npm run release:zip first.`);
  ...
  if (!releaseTag) {
    problems.push(`Zip filename does not follow expected pattern ${PLUGIN_ID}-X.Y.Z.zip`);
  }
